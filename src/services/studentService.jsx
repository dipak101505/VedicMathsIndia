/**
 * This service is used to get the student data for a specific student
 * It is used to display the student data in the student page
 * It is also used to display the student data in the student page
 * It is also used to display the student data in the student page
 */
...
export const upsertStudent = async (studentData) => {
  ...
  const params = {
    ...
    Item: {
      ...
      GSI1SK: `STUDENT#${normalizedEmail}`,
    },
  };

  await dynamoDB.send(new PutCommand(params));
  return { id: normalizedEmail };
};
...
export const ensureVideoCacheTableExists = async () => {
  // Consistent naming
  try {
    await ddbClient.send(
      new DescribeTableCommand({ TableName: videoCacheTableName }),
    ); // Use ddbClient
    ...
    if (error.name === "ResourceNotFoundException") {
      ...
      try {
        ...
        while (!tableActive) {
          try {
            const describeTableResponse = await ddbClient.send(
              new DescribeTableCommand({ TableName: videoCacheTableName }),
            );
  ...
  try {
    ...
    const updatedAttendance = {
      ...
      [subject]: {
        ...
        [topic]: [...(currentAttendance[subject]?.[topic] || []), newRecord],
      },
    };
